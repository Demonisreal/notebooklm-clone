...
describe('toWav', () => {
	...
	it('puts a riff wave header in front', () => {
	...
	it('writes the sample rate, otherwise playback runs too fast or too slow', () => {
		const wav = toWav(pcm, 24000);
	...
	it('reports the size of the payload', () => {
		const wav = toWav(pcm, 24000);
		expect(wav.readUInt32LE(40)).toBe(pcm.length);
		expect(wav.readUInt32LE(4)).toBe(pcm.length + 36);
	});

	it('works out the duration', () => {
